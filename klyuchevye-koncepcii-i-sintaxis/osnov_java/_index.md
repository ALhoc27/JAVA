...
У всех классов исключений есть общий класс-предок `Throwable`, отнего наследуются классы `Error` и `Exception`, базовые для всех прочих.
...
-  **Exception** (исключения: проверяемые / не проверяемые)
...
-  **Checked Exceptions** (проверяемые исключения): компилятор требует их обработки (например, `IOException`, `SQLException`).
...
**Error (Ошибки)** – в Java обозначают серьезные проблемы, которые обычно не могут быть восстановлены.

*Программисты обычно не обрабатывают такие ошибки, так как они указывают на нерешаемые проблемы в окружении выполнения приложения.*

:::

:::tip 

**Exception (Исключения)** – в Java представляют собой события, которые возникают во время выполнения программы и могут быть обработаны.

:::

**java.lang.Exception.** Могут быть разделены на проверяемые **(checked)** и не проверяемые **(unchecked)** исключения.