...
-  Исполнение байт-кода, который хранится в `.class` файлах (**результат работы компилятора**).

-  Управление памятью, включая сборку мусора (Garbage Collection).
...
### 1\. [comment:BY0PL]Перегрузка методов[/comment] (***полиморфизм на уровне компиляции***)
...
### [comment:TOx2y]2\. Переопределение методов[/comment] (***Полиморфизм на уровне выполнения***)
...
#### Рассмотрим на примере основные концепции java в контексте обьектов, экземпляров класса и полей:

![](./osnov_java-6.png)

#### Классы

Класс -- это шаблонная конструкция, которая позволяет описать в программе [comment:SCwUf]объект[/comment], его свойства ([comment:y5GCx]атрибуты или поля класса[/comment]) и поведение ([comment:KsASv]методы[/comment] класса).

[html:iframe]

<mark style="background-color: transparent; color: black; font-family: font-family: -apple-system, BlinkMacSystemFont, Roboto, Helvetica Neue, sans-serif !important; font-size: 17px !important;"><i>(это как проект здания, в проекте всегда есть атрибуты (поля), так же могут быть методы / инструкции, - действия с теми или иными полями, например какие - либо расчеты</i></mark>
...
*  {% colwidth=[730] %}
   ...
   6\. **Уровень сетевой безопасности (Network Security)**: Этот уровень фокусируется на обеспечении безопасности на уровне сетевой инфраструктуры. Это включает в себя использование протоколов безопасности, шифрование данных и аутентификацию.

---

*  {% colwidth=[730] %}

   7\. **Уровень приложения сети (Application-Level Network Security)**: Java-приложения могут реализовать собственные меры безопасности для обеспечения конфиденциальности и целостности данных при обмене по сети.
...
*  {% colwidth=[730] %}
   ...
   8\. **Уровень безопасности приложений (Application Security)**: Разработчики могут внедрять собственные меры безопасности в приложения, такие как аутентификация пользователей, управление сеансами и аудит действий.

---

*  {% colwidth=[730] %}

   9\. **Уровень системы (System-Level Security)**: В зависимости от того, на какой операционной системе Java-приложение работает, системные политики безопасности могут быть также применены для обеспечения безопасности.

---

*  {% colwidth=[730] %}

   10\. **Уровень аудита и журналирования (Audit and Logging Security)**: Ведение журналов и аудит действий приложения может быть важным средством для обнаружения и реагирования на потенциальные угрозы безопасности.
