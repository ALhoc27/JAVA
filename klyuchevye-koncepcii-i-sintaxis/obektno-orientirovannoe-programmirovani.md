...
-  `protected` **\- Доступен только внутри того же пакета (любые наследники)**

-  `default` **\- Доступен только внутри того же пакета (наследники того же пакета)**
...
Статические методы в Java -- **это методы**, **которые принадлежат классу**, а не конкретному экземпляру этого класса. Они могут быть вызваны без необходимости создания объекта класса. Давайте рассмотрим их подробнее.
...
[tab:Пример статического метода::]
...
Статические методы могут использоваться в различных сценариях:

1. **Утилитарные функции**: Статические методы часто используются для создания вспомогательных (утилитарных) функций, которые не требуют состояния экземпляра. Например, в классе `Math` есть много статических методов (например, `Math.sqrt()`).

2. **Фабричные методы**: Статические методы могут использоваться для создания экземпляров объектов с некоторой логикой создания. Например, вы можете использовать статический метод для создания объекта определённого класса на основе переданных параметров.

3. **Доступ к статическим переменным**: Статические методы могут обращаться к статическим переменным класса и изменять их значения.

:::note:true Пример использования статического метода для создания объекта

```java
public class Person {
    private String name;

    // Статический метод для создания объекта Person
...
public class Main {
    public static void main(String[] args) {
        ...
        Person person = Person.createPerson("John");
        person.display(); // Вывод: Name: John
    }
}
```
...
-  **Нет доступа к нестатическим членам**: Статические методы не могут напрямую обращаться к нестатическим переменным и методам класса. Чтобы обратиться к нестатическим членам, необходимо создать экземпляр класса.

-  **Не могут быть переопределены**: Статические методы не могут быть переопределены в подклассах. Если подкласс объявляет метод с тем же именем и параметрами, это будет рассматриваться как перегрузка, а не переопределение.

#### Модификаторы доступа статических методов

[image:./obektno-orientirovannoe-programmirovani-10.png:::0,0,100,100:83:]

Статические методы в Java поддерживают все модификаторы доступа.
...
[tab:Пример использования статических блоков::]
