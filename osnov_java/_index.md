...
Класс -- это шаблонная конструкция, которая позволяет описать в программе [comment:SCwUf]объект[/comment], его свойства (атрибуты или поля класса) и поведение (методы класса).

[html:iframe]

<mark style="background-color: transparent; color: #333; padding: 3px 6px; border-radius: 3px; font-size: 16px; font-family: -apple-system, BlinkMacSystemFont, Roboto, Helvetica Neue, Arial, sans-serif;"><i>(это как проект здания, в проекте всегда есть атрибуты (поля), так же могут быть методы (инструкции) (действия с теми или иными полями, например расчет стоимости) - методы)</i></mark>
