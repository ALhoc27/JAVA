---
order: 1
title: 1. Ключевые концепции и синтаксис
---

### 1\. **Классы и объекты**

Java является объектно-ориентированным языком программирования, где основными строительными блоками являются **классы** и **объекты**.

-  **Класс** -- это шаблон или чертеж для создания объектов. Он описывает свойства (поля) и поведение (методы), которые будут у объектов.

-  **Объект** -- это экземпляр класса. Объект обладает конкретными значениями полей и может вызывать методы.

:::note:true Пример:

В этом примере создадим базовый класс `Animal` и два подкласса -- `Dog` и `Cat`. Каждый из этих классов будет реализовывать метод `sound()` по-своему.

```java
// Класс Car
public class Car {
    // Поля класса
    String model;
    int year;

    // Конструктор
    public Car(String model, int year) {
        this.model = model;
        this.year = year;
    }

    // Метод
    public void start() {
        System.out.println(model + " is starting");
    }
}

// Использование класса и создание объекта
public class Main {
    public static void main(String[] args) {
        Car car1 = new Car("Toyota", 2020);
        car1.start();  // Выведет: Toyota is starting
    }
}
```

:::

### 2\. **Переменные**

Java поддерживает несколько типов переменных:

-  **Члены класса или поля класса (атрибуты)** -- это переменные, объявленные в классе, которые хранят состояние объекта.

-  **Локальные переменные** -- объявлены внутри методов и существуют только в их пределах.

-  **Статические переменные** -- переменные, принадлежащие классу, а не конкретному объекту.

-  **Финальные переменные** -- означает, что переменная, помеченная как `final`, не может быть изменена после присвоения значения.

### 3\. **Типы данных**

Java -- это статически типизированный язык, что означает, что каждый объект и переменная должны иметь определенный тип данных. В Java существуют два основных типа данных:

{% table %}

---

*  {% colwidth=[142] isHeader=true %}

   Тип

*  {% colwidth=[260] isHeader=true %}

   Размер

*  {% colwidth=[283] isHeader=true %}

   Диапазон

*  {% colwidth=[161] isHeader=true %}

   По умолчанию:

---

*  {% colwidth=[142] %}

   1. [comment:KxMVb]byte[/comment]

*  {% colwidth=[260] %}

   8 бит (1 байт)

*  {% colwidth=[283] %}

   от -128 до 127

*  {% colwidth=[161] %}

   0

---

*  {% colwidth=[142] %}

   2. [comment:av8Nj]short[/comment]

*  {% colwidth=[260] %}

   16 бит (2 байта)

*  {% colwidth=[283] %}

   от -32,768 до 32,767

*  {% colwidth=[161] %}

   0

---

*  {% colwidth=[142] %}

   3. [comment:AmVpf]int[/comment]

*  {% colwidth=[260] %}

   32 бита (4 байта)

*  {% colwidth=[283] %}

   [comment:tnIUN]от -2^31 до 2^31-1 [/comment]

*  {% colwidth=[161] %}

   0

---

*  {% colwidth=[142] %}

   4. [comment:8Vqnd]long[/comment]

*  {% colwidth=[260] %}

   64 бита (8 байт)

*  {% colwidth=[283] %}

   от -2^63 до 2^63-1

*  {% colwidth=[161] %}

   0L

---

*  {% colwidth=[142] %}

   5. [comment:sX1hS]float[/comment]

*  {% colwidth=[260] %}

   32 бита (4 байта)

*  {% colwidth=[283] %}

   [comment:UGuOb]от \~1.4e−45 до \~3.4e+38[/comment]

*  {% colwidth=[161] %}

   0\.0f

---

*  {% colwidth=[142] %}

   6. [comment:o6vZm]double[/comment]

*  {% colwidth=[260] %}

   64 бита (8 байт)

*  {% colwidth=[283] %}

   [comment:KzT2U]от \~4.9e−324 до \~1.8e+308[/comment]

*  {% colwidth=[161] %}

   0\.0d

---

*  {% colwidth=[142] %}

   7. [comment:c94GX]boolean[/comment]

*  {% colwidth=[260] %}

   [comment:2IQZV]Выделяется минимум 1 байт[/comment]

*  {% colwidth=[283] %}

   `true` или `false`

*  {% colwidth=[161] %}

   `false`

---

*  {% colwidth=[142] %}

   8. [comment:5NsEE]char[/comment]

*  {% colwidth=[260] %}

   16 бит (2 байта)

*  {% colwidth=[283] %}

   от `\u0000` (0) до `\uffff` (65,535)

*  {% colwidth=[161] %}

   `\u0000` или 0

{% /table %}

-  **Простые (примитивные) типы**: `int`, `double`, `float`, `boolean`, `char`, `byte`, `short`, `long`.

   `byte`: 8-битное целое число (от -128 до 127)

   `short`: 16-битное целое число (от -32,768 до 32,767)

   `int`: 32-битное целое число (от -2^31 до 2^31-1)

   `long`: 64-битное целое число (от -2^63 до 2^63-1)

   `float`: 32-битное число **с плавающей запятой** (с точностью до \~7 знаков)

   `double`: 64-битное число **с плавающей запятой** (с точностью до \~15 знаков)

   `boolean`: принимает значения `true` или `false`

   `char`: 16-битный символ Unicode Диапазон значений: от `\u0000` (или 0) до `\uffff` (или 65,535)

   

   

   

   

   

   

   

   

-  `short`: 16-битное целое число (от -32,768 до 32,767)

-  `int`: 32-битное целое число (от -2^31 до 2^31-1)

-  `long`: 64-битное целое число (от -2^63 до 2^63-1)

-  `float`: 32-битное число с плавающей запятой (с точностью до \~7 знаков)

-  `double`: 64-битное число с плавающей запятой (с точностью до \~15 знаков)

-  `boolean`: принимает значения `true` или `false`

-  `char`: 16-битный символ Unicode (от '\\u0000' до '\\uffff')

-  **Ссылочные типы**: это классы и массивы, такие как `String`, `Array`, и пользовательские классы.

:::note:true Пример:

```java
int num = 5;           // Примитивный тип
String text = "Hello"; // Ссылочный тип
```

:::

### 4\. **Методы**

Метод -- это блок кода, который выполняет определенное действие. Методы могут принимать параметры и возвращать значения.

:::note:true Синтаксис метода:

```java
public возвращаемый_тип имя_метода(параметры) {
    // Тело метода
    return значение;
}
```

Пример метода:

```java
public int addNumbers(int a, int b) {
    return a + b;
}
```

:::

### 5\. **Модификаторы доступа**

Java предоставляет модификаторы доступа для ограничения видимости классов, методов и полей:

-  `public` -- доступен для всех.

-  `private` -- доступен только внутри того же класса.

-  `protected` -- доступен в том же пакете или в подклассах.

-  Без модификатора (package-private) -- доступен только внутри того же пакета.

:::note:true Пример:

```java
public class Car {
    private String model; // Доступ только внутри класса
    public int year;      // Доступен везде
}
```

:::

### 6\. **Конструкторы**

Конструктор -- это специальный метод, который вызывается при создании объекта. Конструктор используется для инициализации объекта.

:::note:true Пример конструктора:

```java
public class Car {
    String model;
    int year;
    // Конструктор
    public Car(String model, int year) {
        this.model = model;
        this.year = year;
    }
}
```

:::

### 7\. **Наследование**

Java поддерживает наследование, это один из ключевых принципов объектно-ориентированного программирования (ООП), позволяющий одному классу использовать свойства и методы другого класса. В Java наследование достигается с помощью ключевого слова `extends`.

:::note:true Пример наследования:

```java
// Базовый класс (родительский класс)
class Animal {
    public void eat() { // Метод базового класса
        System.out.println("This animal is eating");
    }
}

class Dog extends Animal { // Подкласс (наследующий класс, дочерний класс)
    // Метод подкласса
    public void bark() {
        System.out.println("Собака лаит");
    }
}

public class Main {
    public static void main(String[] args) {  
        Dog dog = new Dog(); // Создание объекта подкласса Dog

        // Вызов метода базового класса
        dog.eat();  // Выведет: This animal is eating

        // Вызов метода подкласса
        dog.bark(); // Выведет: Собака лаит
    }
}
```

### Объяснение:

-  **Класс** `Animal` является базовым классом, содержащим метод `eat()`, который может быть вызван любым его подклассом.

-  **Класс** `Dog` является подклассом и наследует все методы и поля класса `Animal`. В данном примере он наследует метод `eat()`.

-  Подкласс `Dog` также может иметь свои собственные методы, такие как `bark()`.

#### Пример переопределения методов (Override)

Подклассы могут **переопределять** методы базового класса для изменения их поведения.

```java
// Базовый класс (родительский класс)
class Animal {
    // Метод базового класса
    public void sound() {
        System.out.println("Animal makes a sound");
    }
}

// Подкласс (наследующий класс)
class Dog extends Animal {
    // Переопределение метода базового класса
    @Override
    public void sound() {
        System.out.println("Dog barks");
    }
}

public class Main {
    public static void main(String[] args) {
        // Создание объекта подкласса Dog
        Dog dog = new Dog();
        
        // Вызов переопределенного метода
        dog.sound();  // Выведет: Dog barks
    }
}
```

### Объяснение:

-  В классе `Dog` метод `sound()` из класса `Animal` был переопределен с помощью аннотации `@Override`. Теперь при вызове метода `sound()` у объекта `Dog` будет выведено «Dog barks», а не «Animal makes a sound».

:::

### 8\. **Полиморфизм**

Java поддерживает полиморфизм, это возможность одного метода работать с разными типами объектов. Он позволяет одному и тому же методу обрабатывать разные объекты по-разному, в зависимости от типа объекта, на котором он вызывается. Полиморфизм реализуется через наследование и интерфейсы.

:::note:true Пример полиморфизма:

В этом примере создадим базовый класс `Animal` и два подкласса -- `Dog` и `Cat`. Каждый из этих классов будет реализовывать метод `sound()` по-своему.

```java
// Базовый класс
class Animal {
    public void sound() {
        System.out.println("The animal makes a sound");
    }
}

// Подкласс Dog
class Dog extends Animal {
    @Override
    public void sound() {
        System.out.println("The dog barks");
    }
}

// Подкласс Cat
class Cat extends Animal {
    @Override
    public void sound() {
        System.out.println("The cat meows");
    }
}

public class Main {
    public static void main(String[] args) {
        // Полиморфизм через ссылки на базовый класс
        Animal myAnimal = new Animal(); // Базовый объект
        Animal myDog = new Dog();       // Объект типа Dog
        Animal myCat = new Cat();       // Объект типа Cat

        // Вызов метода sound() для каждого объекта
        myAnimal.sound();  // Выведет: The animal makes a sound
        myDog.sound();     // Выведет: The dog barks
        myCat.sound();     // Выведет: The cat meows
    }
}
```



:::

### 9\. **Абстракция**

**Абстракция** -- одна из ключевых концепций объектно-ориентированного программирования (ООП). Она заключается в создании простых интерфейсов для сложных систем, скрывая детали реализации. Абстракция позволяет определить общие характеристики объектов, при этом конкретная реализация этих характеристик остается скрытой.